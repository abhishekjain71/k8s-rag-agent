...
## Load evironment variables explicitly  form the root directory
env_path= os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".env"))
load_dotenv(dotenv_path=env_path)

### Initialize the logfire--
try:
    token= os.getenv("LOGFIRE_TOKEN")
    if not token:
        print("ERROR: LOGFIRE TOKEN is empty or None")
                    ...
                    if sources:
                        with st.expander(f"📄 View Retrieved Context ({len(sources)} chunks)"):
                            ...
                            for i, source in enumerate(sources):
                                ...
                                # Small preview for collapsed chunk
                                preview = " ".join(source.split())[:100]
